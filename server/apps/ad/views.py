            ...
            for value in values:
                try:
                    ...
                    if facet['name'] == 'groups':
                        ...
                        value_serializer['label'] = InterestGroup.objects.filter(Q(memberships__user=user) | Q(owner=user)).get(slug=value_serializer['name']).name #, members__in=user.pk
    ...
    for name, values in facets_fields:
        ...
        for value in values:
            try:
                ...
                if facet['name'] == 'groups':
                    if value_serializer['name'] == 'public': raise InterestGroup.DoesNotExist
                    value_serializer['label'] = InterestGroup.objects.\
                        filter(Q(memberships__user=user) | Q(owner=user)).\
                        get(slug=value_serializer['name']).name
...
class DetailAdView(DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        #TODO: What happens when the first
        if obj.groups.first().slug != 'public':
            # Validate if user has permission to view this ad.
            if not self.request.user.is_authenticated() or \
                    not InterestGroup.objects.filter(pk__in=obj.groups.all().values_list('id')).filter(
                                    Q(memberships__user=self.request.user) | Q(owner=self.request.user)).exists():
