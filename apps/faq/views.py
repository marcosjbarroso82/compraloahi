...
# TODO: Crear mixins para poder llevar "topics" a todas las vistas
...
class FaqIndexView(ListView):
    ...
    def get_context_data(self, **kwargs):
        data = super(FaqIndexView, self).get_context_data(**kwargs)

        last_updated = Question.objects.filter(status=1).order_by('updated_on')[:5]

        max_score = QuestionScore.objects.all().order_by('score')[:20].values_list('question')

        popular_questions = Question.objects.filter(id__in=max_score, status=1)

        try:
            topic = Topic.objects.get(slug='preguntas-frecuentes')
            common_question = topic.questions.filter(status=1)
        except Topic.DoesNotExist:
            common_question = []

        # TODO: popular questions divider two for frontend
        data.update({'last_updated': last_updated,
                     'popular_questions': popular_questions[:10],
                     'popular_questions2': popular_questions[10:],
...
class QuestionOverviewList(ListView):
    ...
    def get_question_score(self, **kwargs):
        ...
        data.update({'questions': questions, 'topics': Topic.objects.all()})
        return data
